...
async function handleRequest(
    ...
    try {
        ...
        await DB.prepare(updateLoginSql).bind(lastLoginTime, user.user_id)
            .run();

        const token = btoa(user.user_id + "-" + lastLoginTime);
        const deviceInfo = request.headers.get("User-Agent") || "unknown";
        const insertTokenSql =
            "INSERT INTO USER_TOKENS (token, user_id, device_info, login_time) VALUES (?, ?, ?, ?)";
        await DB.prepare(insertTokenSql).bind(
            token,
            user.user_id,
            deviceInfo,
            lastLoginTime,
